...
class VideoStream:
    def __init__(self, file_path, filename, file_id=None, user_id=None):
...
def upload_video():
    try:
        ...
        filename = secure_filename(f"video_{int(time.time())}.mp4")
        ...
        if download_telegram_file(file_path, local_path):
            ...
            active_streams[stream_id] = VideoStream(local_path, filename, file_id, None)
